...
-- Step 1: Rename table when upgrading an older database.
ALTER TABLE IF EXISTS public.whatsapp_sessions RENAME TO chat_sessions;
...
CREATE INDEX IF NOT EXISTS chat_sessions_channel_idx ON public.chat_sessions(channel);
CREATE INDEX IF NOT EXISTS chat_sessions_phone_idx ON public.chat_sessions(phone);
CREATE INDEX IF NOT EXISTS chat_sessions_channel_phone_idx ON public.chat_sessions(channel, phone);
...
BEGIN
  ...
  IF policy_count > 0 THEN
    ALTER POLICY whatsapp_sessions_self_select_rls ON public.chat_sessions RENAME TO chat_sessions_self_select_rls;
  END IF;
END $$;
